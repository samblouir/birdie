...
- If no spans are inserted after `max_attempts`, we force a single masked span so the sample stays trainable.
...
try:
	# Optional C++ fast path (built at install time, or not at all).
	from birdie_rl.objectives import _infilling_fast as _infilling_fast  # type: ignore[attr-defined]
except Exception:  # noqa: BLE001 - optional extension may fail to import for many reasons
	_infilling_fast = None
...
class InfillingConfig(BaseObjectiveConfig):
	...
	fast_backend_num_candidates: int = 1
	...
	def __post_init__(self):
		...
		self.fast_backend_num_candidates = int(self.fast_backend_num_candidates)
		if self.fast_backend_num_candidates < 1:
			self.fast_backend_num_candidates = 1
...
class InfillingObjective(BaseObjective):
	...
	def __init__(self, config: InfillingConfig) -> None:
		...
		if not isinstance(config.fast_backend_num_candidates, int) or config.fast_backend_num_candidates < 1:
			raise ValueError(f"fast_backend_num_candidates must be >= 1. Got: {config.fast_backend_num_candidates}")
		...
		self._placeholder_token_cache: dict[int, list[int]] = {}

	def _get_placeholder_tokens(self, placeholder_idx: int, config: InfillingConfig) -> list[int]:
		if placeholder_idx in self._placeholder_token_cache:
			return self._placeholder_token_cache[placeholder_idx]
		ph_str = f"{config.mask_prefix}{placeholder_idx}{config.mask_suffix}"
		ph_toks = self.safe_cast_to_list(self.tokenizer.encode(ph_str))
		self._placeholder_token_cache[placeholder_idx] = ph_toks
		return ph_toks

	def _get_placeholder_bank(self, max_mask_spans: int, config: InfillingConfig) -> list[list[int]]:
		return [self._get_placeholder_tokens(i, config) for i in range(int(max_mask_spans))]
	...
	def build_input_and_labels(
		...
		paradigm_end_toks = self.tokenized_paradigm_end

		if _infilling_fast is not None and config.use_fast_backend:
			try:
				placeholder_bank = self._get_placeholder_bank(config.max_mask_spans, config)
				try:
					(
						status,
						used_fallback,
						...
						masked_count,
						original_length,
					) = _infilling_fast.build_infilling(  # type: ignore[union-attr]
						encoded_input,
						int(config.remaining_space),
						int(config.rng_seed),
						float(config.minimum_corruption_rate),
						float(config.maximum_corruption_rate),
						float(config.mean_tokens_per_span),
						float(config.minimum_mean_tokens_per_span),
						float(config.maximum_mean_tokens_per_span),
						int(config.max_mask_spans),
						int(config.max_attempts),
						prompt_toks,
						paradigm_end_toks,
						placeholder_bank,
						int(config.fast_backend_num_candidates),
					)
				except TypeError:
					# Backward compatibility with an older extension build that doesn't
					# accept fast_backend_num_candidates.
					(
						status,
						used_fallback,
						input_bytes,
				...
				if int(status) == 0:
					...
					label_ids = np.frombuffer(label_bytes, dtype=np.int32)
					unused_input_ids = np.frombuffer(unused_bytes, dtype=np.int32)
					unused_input_str = tokenizer.decode(unused_input_ids.tolist()) if unused_input_ids.size else ""
					return {
						"status": "ok",
						"objective": "Infilling (fallback, forced mask)" if used_fallback else "Infilling",
						"input_ids": input_ids,
						"label_ids": label_ids,
				...
				if int(status) == 1:
					return {
						...
						"objective": "Infilling",
						"message": "Not enough remaining_space for infilling fast path.",
					}
			except Exception:
				# Fall back to the pure-Python implementation below.
				pass
		...
		for attempt_i in range(config.max_attempts):
			...
			label_len_total = len(paradigm_end_toks)
			...
			while text_idx < max_n_tokens_to_process and placeholders_inserted_count < config.max_mask_spans:
				...
				current_labels_len = label_len_total
				...
				if self.np_rng.uniform() < prob_to_mask and (max_n_tokens_to_process - text_idx >= span_len_to_mask):
					...
					ph_toks = self._get_placeholder_tokens(placeholders_inserted_count, config)

					prospective_input_len = current_input_len + len(ph_toks)
					prospective_label_block_len = len(ph_toks) + len(snippet_to_mask)
					prospective_total_labels_len = current_labels_len + prospective_label_block_len
					
					if (prospective_input_len + prospective_total_labels_len) <= config.remaining_space:
						current_input_ids_list.extend(ph_toks)
						label_ids_list.extend(ph_toks)
						label_ids_list.extend(snippet_to_mask)
						label_len_total += prospective_label_block_len
			...
			if placeholders_inserted_count > 0:
				final_label_ids_list = label_ids_list
				if paradigm_end_toks:
					if len(current_input_ids_list) + len(final_label_ids_list) + len(paradigm_end_toks) <= config.remaining_space:
						final_label_ids_list.extend(paradigm_end_toks)
			...
			# Ensure fallback produces a *trainable* sample (non-empty label_ids).
			# If no spans were inserted after `max_attempts`, force a single masked span
			# so the packer can accept the sample and the worker won't spin.
			ph_str = f"{config.mask_prefix}0{config.mask_suffix}"
			ph_toks = self.safe_cast_to_list(tokenizer.encode(ph_str))
			paradigm_end_toks = self.tokenized_paradigm_end

			budget_for_text_tokens = (
				config.remaining_space
				- len(prompt_toks)
				- (2 * len(ph_toks))
				- len(paradigm_end_toks)
			)
			if budget_for_text_tokens <= 0:
				return {
					"status": "not_enough_space",
					"objective": "Infilling",
					"message": "Not enough remaining_space for forced-mask fallback.",
				}

			total_text_tokens_to_use = int(min(max_n_tokens_to_process, budget_for_text_tokens))
			total_text_tokens_to_use = max(1, total_text_tokens_to_use)

			mask_len = int(max(1, min(sample_span_length(0), total_text_tokens_to_use)))
			masked_tokens = encoded_input[:mask_len]
			unmasked_tokens = encoded_input[mask_len:total_text_tokens_to_use]

			final_input_ids.extend(ph_toks)
			final_input_ids.extend(unmasked_tokens)

			final_label_ids_list = list(ph_toks)
			final_label_ids_list.extend(masked_tokens)
			if paradigm_end_toks:
				if len(final_input_ids) + len(final_label_ids_list) + len(paradigm_end_toks) <= config.remaining_space:
					final_label_ids_list.extend(paradigm_end_toks)

			unused_input_ids_list = encoded_input[total_text_tokens_to_use:]
			unused_input_str = tokenizer.decode(unused_input_ids_list)
			return {
				"status": "ok", 
				"objective": "Infilling (fallback, forced mask)",
				"input_ids": np.array(final_input_ids, dtype=np.int32),
				"label_ids": np.array(final_label_ids_list, dtype=np.int32),
				"unused_input_string": unused_input_str,
				"unused_input_ids": np.array(unused_input_ids_list, dtype=np.int32),
				"masked_count": len(masked_tokens),
				"original_length": total_text_tokens_to_use,
			}
