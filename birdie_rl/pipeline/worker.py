...
def _default_text_source(
	worker_id: int,
	total_workers: int,
	split: str = "train",
	rng_seed: int = 0,
	deterministic_worker_rng: bool = False,
):
	pid = os.getpid()
	# print(f"[_default_text_source Worker {worker_id} PID {pid}] Initializing for TinyStories, split: {split}.", flush=True)

	try:
		# Import lazily so users who provide their own `data_generator` don't need
		# `datasets` (or its transitive deps) to be importable.
		...
		local_rng = np.random.default_rng(rng_seed + worker_id + (0 if deterministic_worker_rng else pid))
...
class Worker:
	def __init__(
		...
		self.deterministic_worker_rng = bool(self.main_config.get("deterministic_worker_rng", False))
		...
		if not callable(data_generator):
			self.data_generator_fn_callable = partial(_default_text_source,
												...
												deterministic_worker_rng=self.deterministic_worker_rng)
		else:
		...
		pid = os.getpid()
		worker_rng_seed = self.base_rng_seed + self.worker_id
		if not self.deterministic_worker_rng:
			worker_rng_seed += pid
		self.rng = np.random.default_rng(worker_rng_seed)
		...
		config_for_internal_packer = {
			...
			"seed": (
				self.base_rng_seed + self.worker_id
				if self.deterministic_worker_rng
				else self.base_rng_seed + self.worker_id + pid + int(time.time()*1000) % 100000
			),
		}
	...
	def _get_objective_instance(self, objective_name: str, static_config_overrides: dict):
		"""
		Returns a cached objective instance keyed only by *static* config.

		Per-sample fields like `remaining_space` and `rng_seed` must NOT be part of
		the cache key (or the cache grows unbounded and can look like a hang due to
		memory blow-up / GC pressure).
		"""

		if static_config_overrides is None:
			static_config_overrides = {}

		if "tokenizer" not in static_config_overrides:
			static_config_overrides["tokenizer"] = self.tokenizer

		def _freeze_for_cache(v):
			if isinstance(v, dict):
				return tuple(sorted((k, _freeze_for_cache(val)) for k, val in v.items()))
			if isinstance(v, (list, tuple)):
				return tuple(_freeze_for_cache(x) for x in v)
			if isinstance(v, set):
				return tuple(sorted(_freeze_for_cache(x) for x in v))
		...
		# Exclude tokenizer object identity from the key; cache is per-worker anyway.
		key_items = tuple(
			(k, _freeze_for_cache(v))
			for (k, v) in sorted(static_config_overrides.items(), key=lambda kv: kv[0])
			if k != "tokenizer"
		)
		cache_key = (objective_name, key_items)

		if cache_key in self.objective_cache:
			return self.objective_cache[cache_key]

		instance = load_objective(objective_name, static_config_overrides)
		self.objective_cache[cache_key] = instance
		return instance
	...
	def _produce_one_sample(self): # This method now aims to produce one *packed sequence*
		...
		static_cfg_overrides = (
			base_config_overrides.copy() if isinstance(base_config_overrides, dict) else {}
		)

		try:
			objective_instance = self._get_objective_instance(objective_name, static_cfg_overrides)
		...
		# Per-sample (dynamic) fields: update the cached instance before calling it.
		remaining_space = self.internal_packer.get_remaining_space(max_or_min="max")
		rng_seed = int(self.rng.integers(0, 2**32 - 1))
		try:
			objective_instance.config.remaining_space = int(remaining_space)
			objective_instance.config.rng_seed = rng_seed
		except Exception:
			# If an objective doesn't expose a mutable config for some reason, fall back
			# to not caching it (should be rare).
			pass
		...
		# Avoid unnecessary copies when objectives already return NumPy arrays.
	...
	def run(self, profile: bool = False):
		seed = self.base_rng_seed + self.worker_id
		if not self.deterministic_worker_rng:
			seed += os.getpid() + int(time.time()*1000) % 100000
